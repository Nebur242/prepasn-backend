      ...
      {fileList.length > 0 && (
        <>
          ...
          <Row justify="space-between">
            <Button disabled={isLoading} onClick={() => setShowUpload(false)}>
              Cancel
            </Button>
            <Button
              onClick={uploadFiles}
              loading={isLoading}
              icon={<Icon type="CloudUploadOutlined" />}
              type="primary"
            >
              Upload {fileList.length} assets to the library
            </Button>
          </Row>
        </>
      )}
...
const AppUploadFiles: FC<IUploadFilesProps> = ({
  files,
  onRemoveClick,
  onAddClick,
}) => {
        ...
        {files.map((item: UploadFile) => {
          return (
            <Col span={8} key={item.uid}>
              <AppUploadFile
                onDeleteClick={(file) => onRemoveClick(file)}
                file={item}
              />
            </Col>
          );
