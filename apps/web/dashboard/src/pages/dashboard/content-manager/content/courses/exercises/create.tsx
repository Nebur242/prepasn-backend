...
const Create = () => {
    ...
    const { data: chapter, isLoading: chapterIsLoading } = useFindOneChapterQuery(chapterId);
    ...
    useEffect(() => {
        if (exerciseIsSuccess) {
            ...
            navigate(`/admin/content-manager/courses/${courseId}/chapters/${chapterId}/exercises/update/${exercise.id}`);
        }
        if (exerciseIsError) message.error('Une erreur est survenue');
    }, [exerciseIsSuccess, exerciseIsError, form, courseId, chapterId, navigate, exercise?.id]);
